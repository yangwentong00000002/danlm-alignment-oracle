class Myfunc1014(object):
    def firstcard(self,hd, cur):
        ...
        if len(dict['Bomb']) > 0:
            for i in dict['Bomb'][1::2]:
                for j in i:
                    if j in hdcopy and poke.index(j[1]) <= 12:
                        hdcopy.remove(j)
    ...
    def getcard(self,hd, numh):
        ...
        dict = {'Single': [], 'Pair': [], 'Trips': [], 'ThreePair': [], 'TwoTrips': [],
                'ThreeWithTwo': [],  'Straight': [], 'Boom': [], 'StraightFlush': [], 'numh': []}
        lnumh = len(numh)
        #print("有", lnumh, "个红桃配")
        colorlist = []  # 花色的牌
        sg = []  # 顺子
        ...
        for i in poke:
            ...
            if list == 0 or list == 'k':
                ...
                dict['Boom'].append(colorlist)
        # 寻找三带二
        if len(dict['Trips']) > 0 and len(dict['Pair']) > 0 and poke.index(dict['Pair'][0]) <= 12:
    ...
    def getcolor(self, dict, cur):
        ...
        if len(dict['Straight']):
            for i in dict['Straight'][1::2]:
                ...
                # print(ls[0])
            return ls[0][0]
        if len(dict['ThreePair']) and poker.index(dict['ThreePair'][0])<=8:
    ...
    def reSort2(self,curRank, getNum):
        poker = ['2', '3', '4', '5', '6', '7', '8', '9', 'T', 'J', 'Q', 'K', 'A', 'B', 'R']
